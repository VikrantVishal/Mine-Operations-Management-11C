...
// connectivity and sync automatically once back online. Uses the
// multi-tab-safe cache (persistentMultipleTabManager) instead of the old
// enableIndexedDbPersistence(), since this is a multi-page app and people
// commonly have several tabs (Overview, Manpower, Production, ...) open
// at once — the single-tab API throws "failed-precondition: could not
// obtain exclusive access" in every tab after the first.
let db;
try {
  ...
  // Falls back to a plain (memory-cache) Firestore instance on browsers
  // without IndexedDB support, or if initializeFirestore was already
  // called for this app elsewhere — app still works online-only.
  db = getFirestore(app);
}
